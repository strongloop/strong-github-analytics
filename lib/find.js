...
exports.handler = function(argv, options) {
  ...
  return exports.findAllIssues(octo, target.owner, target.repo)
    .then(function(issues) {
      debug('Open issues returned: %s', issues.length);
      return exports.findOldIssues(issues, argv);
    }).then(function(issues) {
...
exports.findAllIssues = function(octo, owner, repo) {
  return octo.repos(owner, repo).fetch().then(function(repoInfo) {
    debug('Fetch issues from repository');
    return util.getAllPages(repoInfo.issues.fetch, { state: 'open' });
  });
};

exports.findOldIssues = function(issues, argv) {
  var age = argv.age || 0;
